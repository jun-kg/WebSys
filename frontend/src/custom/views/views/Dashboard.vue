...
const fetchDashboardStats = async () => {
  try {
    ...
    const apiUrl = `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/statistics/dashboard`
    ...
    if (error.name === 'AbortError') {
      console.warn('Dashboard stats fetch timeout')
      ElMessage.warning('データの取得がタイムアウトしました。再試行してください。')
    } else {
