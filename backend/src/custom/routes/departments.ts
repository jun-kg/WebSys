 ...
 * ADMIN: GLOBAL（全社部署閲覧）
 * MANAGER: DEPARTMENT（所属部署閲覧）
 * USER/GUEST: DEPARTMENT（所属部署閲覧）
 */
router.get('/tree',
 ...
 * ADMIN: GLOBAL（全部署閲覧）
 * MANAGER: DEPARTMENT（所属部署のみ閲覧）
 * USER/GUEST: DEPARTMENT（所属部署のみ閲覧）
 */
router.get('/:id',
  checkDepartmentScope({ action: 'DEPT_VIEW', targetDepartmentIdParam: 'id' }),
  async (req: Request, res: Response) => {
  try {
